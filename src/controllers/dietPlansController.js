...
class DietPlansController {
  async getPlans(req, res, next) {
    const result = await dietPlanRepository.list();
    if (!result.success) {
      return next(result);
    }
  ...
  async createNewPlan(req, res, next) {
    ...
    const result = await dietPlanRepository.create(planData);
    if (!result.success) {
      return next(result);
    }
    res.status(result.status).json(result);
  ...
  async getDay(req, res, next) {
    try {
      ...
      const plan = await DietPlan.findById(id, "days");
      if (!plan) {
        return next(new AppError("NOT_FOUND", "Plan not found", 404));
      }
  ...
  async deletePlan(req, res, next) {
    const { id } = req.params;
    const result = await dietPlanRepository.delete(id);
    if (!result.success) {
      return next(result);
    }
    res.status(result.status).json(result);
...
export default new DietPlansController();
